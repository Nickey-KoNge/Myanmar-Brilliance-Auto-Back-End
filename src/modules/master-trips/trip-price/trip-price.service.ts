...
export class TripPriceService {
  ...
  async findAll(query: PaginateTripPriceDto) {
    const { limit = 10, page = 1, search, route_id, vehicle_model_id, station_id, status } = query;
    const queryBuilder = this.tpRepo.createQueryBuilder('trip_price')
      .leftJoinAndSelect('trip_price.route', 'route')
      .leftJoinAndSelect('trip_price.vehicle_model_relation', 'vehicle_model_relation')
    ...
    if (vehicle_model_id) {
      queryBuilder.andWhere('trip_price.vehicle_model_id = :vehicle_model_id', { vehicle_model_id });
    }
    if (station_id) {
      queryBuilder.andWhere('trip_price.station_id = :station_id', { station_id });
    }
    ...
    const hasFilters = !!(search || route_id || vehicle_model_id || station_id || status);
  ...
  private async getOptimizedCount(
    ...
    try {
      const result = await this.tpRepo.query<
        { estimate: string }[]
      >(
        `
