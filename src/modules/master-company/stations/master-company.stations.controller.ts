...
export class MasterCompanyStationsController{
    ...
    @Post()
    async create(@Body() dto:CreateStationsDto, @Req() req: AuthenticatedRequest){
        const userId = req.user?.id || req.user?.staffName || req.user?.email || req.user?.sub || 'Unknown User';
    ...
    @Patch(':id')
    async update(@Param('id') id:string, @Body() dto:UpdateStationsDto, @Req() req: AuthenticatedRequest){
        const userId = req.user?.id || req.user?.staffName || req.user?.email || req.user?.sub || 'Unknown User';
    ...
    // async restore(@Param('auditId') auditId: string, @Req() req: AuthenticatedRequest) {
    //     const userId = req.user?.id || req.user?.staffName || req.user?.email || req.user?.sub || 'Unknown User';
